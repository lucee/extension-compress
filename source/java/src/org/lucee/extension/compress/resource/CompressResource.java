...
public final class CompressResource extends ResourceSupport {
	...
	CompressResource(CompressResourceProvider provider, Compress zip, String path, boolean caseSensitive) {
		...
		if ("/".equals(path)) {
			...
			String[] pn = CFMLEngineFactory.getInstance().getResourceUtil().translatePathName(path);
			this.parent = pn[0];
			this.name = pn[1];
		}
	...
	public InputStream getInputStream() throws IOException {
		CFMLEngineFactory.getInstance().getResourceUtil().checkGetInputStreamOK(this);
	...
	public String getParent() {
		if (Util.isEmpty(parent)) return null;
	...
	public Resource getRealResource(String realpath) {
		realpath = CFMLEngineFactory.getInstance().getResourceUtil().merge(path, realpath);
