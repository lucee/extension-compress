...
public abstract class ResourceSupport implements Resource {
	...
	public void copyFrom(Resource res, boolean append) throws IOException {
		CFMLEngineFactory.getInstance().getIOUtil().copy(res.getInputStream(), this.getOutputStream(append), true, true);
	}

	@Override
	public void copyTo(Resource res, boolean append) throws IOException {
		CFMLEngineFactory.getInstance().getIOUtil().copy(this.getInputStream(), res.getOutputStream(append), true, true);
	}
	...
	public void moveTo(Resource dest) throws IOException {
		CFMLEngineFactory.getInstance().getResourceUtil().moveTo(this, dest);
	}
	...
	public boolean equals(Object obj) {
		...
		if (getResourceProvider().isCaseSensitive()) {
			...
			return CFMLEngineFactory.getInstance().getResourceUtil().getCanonicalPathSilent(this)
					.equals(CFMLEngineFactory.getInstance().getResourceUtil().getCanonicalPathSilent(other));
		}
		if (getPath().equalsIgnoreCase(other.getPath())) return true;
		return CFMLEngineFactory.getInstance().getResourceUtil().getCanonicalPathSilent(this)
				.equalsIgnoreCase(CFMLEngineFactory.getInstance().getResourceUtil().getCanonicalPathSilent(other));
