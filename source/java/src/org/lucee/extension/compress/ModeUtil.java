...
public final class ModeUtil {
	...
	public static int extractPermissions(int mode, boolean log) {
		...
		if (log && (mode != permissionBits)) {
			...
			if (config != null) {
				Log logger = config.getLog("application");
				if (logger != null) {
					logger.warn("mode", "File type bits removed from mode: original=" + toStringMode(mode) + ", extracted=" + toStringMode(permissionBits));
				}
			}
		}
